...
def fetch_real_data(limit=10, query_term="lung cancer"):
    ...
    for study in data['studies']:
        try:
            ...
            is_fda_drug = "Yes" if oversight.get('isFdaRegulatedDrug') is True else "No"
            is_fda_device = "Yes" if oversight.get('isFdaRegulatedDevice') is True else "No"
            ...
            for item in interventions_list:
                if item.get('type') in ['DRUG', 'BIOLOGICAL', 'GENETIC', 'COMBINATION_PRODUCT']:
                    drug_names.append(item.get('name'))
                    if 'otherNames' in item:
                        aliases.extend(item['otherNames'])
            intervention_str = ", ".join(drug_names) if drug_names else "No Drug Listed"
            ...
            primary_outcome_str = "; ".join(prim_measures) if prim_measures else "N/A"
